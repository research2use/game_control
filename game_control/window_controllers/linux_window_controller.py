...
class LinuxWindowController(WindowController):
    ...
    def get_window_geometry(self, window_id):
        ...
        geometry["left"] = int(re.match(r"\s+Absolute upper-left X:\s+([0-9]+)", window_information.split("\n")[2]).group(1))
        geometry["top"] = int(re.match(r"\s+Absolute upper-left Y:\s+([0-9]+)", window_information.split("\n")[3]).group(1))
