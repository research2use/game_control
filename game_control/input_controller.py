...
class InputController:
    ...
    def click_string(
        ...
        **kwargs,
        ...
        self._is_game_launched()
        return self.backend.click_string(
            query_string,
            button=button,
            frame=frame,
            fuzziness=fuzziness,
    ...
    def drag(
        ...
        **kwargs,
        ...
        self._is_game_launched()
        self.backend.drag(
            button=button, x0=x0, y0=y0, x1=x1, y1=y1, duration=duration, **kwargs
        )
    ...
    def drag_screen_region_to_screen_region(
        ...
        **kwargs,
        ...
        self._is_game_launched()
        self.backend.drag_screen_region_to_screen_region(
            button=button,
    ...
    def _initialize_backend(self, backend, **kwargs):
        if backend == InputControllers.PYAUTOGUI:
            from game_control.input_controllers.pyautogui_input_controller import (
                PyAutoGUIInputController,
            )

            return PyAutoGUIInputController(game=self.game, **kwargs)
        elif backend == InputControllers.NATIVE_WIN32:
            from game_control.input_controllers.native_win32_input_controller import (
                NativeWin32InputController,
            )
    ...
    def _is_game_launched(self):
        if not self.game.is_launched:
            raise InputControllerError(
                "InputController cannot be used while the game is not running!"
            )
...
class MouseEvent(SneakySnekMouseEvent):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            event, button=button, direction=direction, velocity=velocity, x=x, y=y
        )
        self.kwargs = kwargs
    ...
    def as_label(self):
        if self.event == MouseEvents.CLICK and self.button == MouseButton.LEFT:
            ...
            self.event == MouseEvents.CLICK_SCREEN_REGION
            and self.button == MouseButton.LEFT
        ):
            return f"Mouse - Left Region Click - {self.kwargs['screen_region']}"
        elif (
            self.event == MouseEvents.CLICK_SCREEN_REGION
            and self.button == MouseButton.RIGHT
        ):
            return f"Mouse - Right Region Click - {self.kwargs['screen_region']}"
        elif (
            self.event == MouseEvents.CLICK_SCREEN_REGION
            and self.button == MouseButton.MIDDLE
        ):
